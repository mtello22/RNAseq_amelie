...
exp_data <- fread("~/GitHub/RNAseq_amelie/data/salmon_quant/Salmon_EstCount_ENSG.tsv")
exp_data <- merge.data.table(x = degs[,.SD, 
...
rnaseq_reads <- exp_data[, .SD, 
                         .SDcols = setdiff(names(exp_data),
                                           c("ensembl_gene_id",
                                             "entrezgene_id"))]
...
Proportional Recovery (Term between parenthesis): This term captures the relative shift of gene expression under treatment compared to the shift caused by FA stress. A value closer to 1 indicates near-complete recovery to Ctrl, while values < 0 indicate expression moving further away from Ctrl.

Biological Magnitude Weighting | Absolute term |: This term weights the proportional recovery by the magnitude of the expression change induced by FA stress. This means that genes with larger FA stress-induced shifts are given more importance.
...
0 < x < 1: Partial recovery, the treatment was moderately effective in reversing FA stress-induced dysregulation.
...
```{r}
temp <- degs[  padj < alpha & 
               WRR_category %in% c("PNS_Recovery", "PNS_Enhanced") & 
               Group == "PNS_vs_FA", 
...
```{r}
temp <- degs[  padj < alpha & 
               WRR_category %in% c("PTS_Recovery", "PTS_Enhanced") & 
               Group == "PTS_vs_FA", 
             .(external_gene_name, log2FoldChange, WRR_category)]
setorder(temp, "log2FoldChange")
temp
...
# This is selecting
cur_treatment <- "PNS_vs_FA"
treatment <- degs[Group == cur_treatment & padj < alpha, external_gene_name]
baseline_ns <- degs[Group == "FA_vs_Ctrl" & padj > alpha, external_gene_name]
treatment <- intersect(treatment, baseline_ns)
treatment <- degs[Group == cur_treatment & external_gene_name %in% treatment,
                     .(external_gene_name, log2FoldChange, padj)]

treatment <- merge.data.table(x = treatment, y = rnaseq_rank[, .(ENSG, PNS_Recovery, PNS_Classification)], 
                              by.x = "external_gene_name", by.y = "ENSG", 
                              all.x = TRUE, all.y = FALSE)
names(treatment) <- c("gene_name", "log2FC", "adjusted_pvalue", "WRR", "Classification")
setorder(treatment, "WRR")

fwrite(treatment, file = paste("~/GitHub/RNAseq_amelie/output/", 
                                cur_treatment, "-specific.tsv", sep = ""), 
...
treatment <- merge.data.table(x = treatment, y = rnaseq_rank[, .(ENSG, PTS_Recovery, PTS_Classification)], 
                              by.x = "external_gene_name", by.y = "ENSG", 
                              all.x = TRUE, all.y = FALSE)
names(treatment) <- c("gene_name", "log2FC", "adjusted_pvalue", "WRR", "Classification")
setorder(treatment, "WRR")
