...
vst_data <- floor(as.matrix(exp_data[, 4:ncol(exp_data)]))
vst_counts <- DESeq2::varianceStabilizingTransformation(vst_data)
...
First we will visualize the changes in expression comparing exposure to FA vs control condition.
...
```{r}
temp <- degs[Group == "FA_vs_Ctrl"]
temp <- temp[, .SD, 
             .SDcols = setdiff(names(temp), 
                               c("external_gene_name", 
                                 "entrezgene_id", 
                                 "Group"))]

temp <- visualize_degs(temp, alpha, log2FC, top_exp = 5)
temp[[1]]
```
...
First we will visualize the changes in expression comparing exposure to PNS vs control condition.

We will first generate a histogram of the raw p-values for all genes. If the H0 is true, we would expect to see an uniform distribution of pvalues. A peak at low zeroes indicate we reject the H0.

```{r}
temp <- degs[Group == "PNS_vs_CTRL"]
temp <- temp[, .SD, 
             .SDcols = setdiff(names(temp), 
                               c("external_gene_name", 
                                 "entrezgene_id", 
...
temp <- visualize_degs(temp, alpha, log2FC)
temp[[1]]
```
Then we visualize the expression changes using a volcano plot.
...
First we will visualize the changes in expression comparing exposure to PTS vs control condition.

We will first generate a histogram of the raw p-values for all genes. If the H0 is true, we would expect to see an uniform distribution of pvalues. A peak at low zeroes indicate we reject the H0.

```{r}
temp <- degs[Group == "PTS_vs_CTRL"]
temp <- temp[, .SD, 
             .SDcols = setdiff(names(temp), 
                               c("external_gene_name", 
                                 "entrezgene_id", 
...
First we will visualize the changes in expression comparing exposure to PNS vs FA condition.

We will first generate a histogram of the raw p-values for all genes. If the H0 is true, we would expect to see an uniform distribution of pvalues. A peak at low zeroes indicate we reject the H0.

```{r}
temp <- degs[Group == "PNS_vs_FA"]
temp <- temp[, .SD, 
             .SDcols = setdiff(names(temp), 
                               c("external_gene_name", 
                                 "entrezgene_id", 
...
First we will visualize the changes in expression comparing exposure to PTS vs FA condition.

We will first generate a histogram of the raw p-values for all genes. If the H0 is true, we would expect to see an uniform distribution of pvalues. A peak at low zeroes indicate we reject the H0.

```{r}
temp <- degs[Group == "PTS_vs_FA"]
temp <- temp[, .SD, 
             .SDcols = setdiff(names(temp), 
                               c("external_gene_name", 
                                 "entrezgene_id", 
...
ggplot(exp_percent, 
  ...
  theme_bw() +
  facet_grid(~Group) +
  scale_x_continuous(labels = label_percent(scale = 1), limits = c(-200, 200))+
